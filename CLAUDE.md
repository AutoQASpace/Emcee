...
Плагин вкомпиливает исходники модулей Emcee, забирая их **с GitHub** (`AutoQASpace/Emcee`, ветка задачи) — локальные коммиты Emcee плагину не видны, пока ветка не запушена. Каждый релиз Emcee = коммит + пуш ветки + перепин (`swift package update Emcee` в плагине) + пересборка + перезаливка плагина. Локальные `../пути` в манифестах — только как незакоммиченная локальная итерация, в ремоут не уезжают. Детали и ловушки — `../emcee-report-plugin/CLAUDE.md`.
